...
double** create_pade_matrix(int n)
{
    ...
    // #pragma acc parallel create(matrix[0:n][0:n])
    // {
    for (int i = 0; i < n; i++)
    {   
        // #pragma acc loop
...
double* create_zero_array(int n)
{
    ...
    #pragma acc parallel loop num_gangs(1000) create(arr[0:n]) copyout(arr[0:n])
    for (int i = 0; i < n; i++)
        arr[i] = 0;
    return arr;
}
...
double* create_pade_array(int n, double ll, double ul)
{
    ...
    // #pragma acc parallel loop create(arr[0:n]) copyout(arr[0:n]) firstprivate(h, n, x)
    for (int i = 1; i < n - 1 ; i++)
    {
        x = ll + i*h;
