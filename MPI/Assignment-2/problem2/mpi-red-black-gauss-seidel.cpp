...
bool convergence(double** &mesh, double** &new_mesh, int rows, int cols)
{
    ...
    double aggregate = pow((aggregate_val/aggregate_norm), 0.5);
...
void Gauss_Seidel_RB(double** &mesh, double** &new_mesh, int grid_points, int mod_grid_points, int local_grid_points, int ll, int ul, double delta)
{
    int myid, nprocs;
    MPI_Comm_size(MPI_COMM_WORLD, &nprocs);
    MPI_Comm_rank(MPI_COMM_WORLD, &myid);

    // cout << myid << ":" << "New Mesh created" << endl;
    ...
    // init_local_mesh(new_mesh, grid_points, local_grid_points, ll, ul, delta);
    // update_ghost_points(new_mesh, grid_points, local_grid_points);
    // cout << myid << ":" << "Local Mesh Initialized successfully" << endl;
    ...
    for (int i = 1; i < local_grid_points+1; i++)
    {
        for (int j = 1; j < grid_points-1; j++)
        {
            if ((i + j) % 2 == 0)
            {
                new_mesh[i][j] = (1.0/4) * ((new_mesh[i+1][j] + new_mesh[i-1][j] + new_mesh[i][j+1] + new_mesh[i][j-1]) + (pow(delta, 2)*q(i, j, ll, delta)));
            }          
        }   
    }

    // Red Right Boundary Condition
    for (int i = 1; i < local_grid_points + 1; i++)
    {
        if ((i + grid_points - 1 ) % 2 == 0)
        {
    ...
    // Black Right Boundary Condition
    for (int i = 1; i < local_grid_points + 1; i++)
    {
        if ((i + grid_points - 1 ) % 2 == 1)
        {
            new_mesh[i][grid_points-1] = (1.0/3)*(4*new_mesh[i][grid_points-2] - new_mesh[i][grid_points-3]);
        }
    }
    ...
    update_ghost_points(new_mesh, grid_points, local_grid_points);
    // cout << myid << ":" << "Updated ghost points successfully" << endl;
...
int main(int argc, char** argv)
{
    ...
    *****************************/
    ...
    do
    {
        ...
        Gauss_Seidel_RB(local_mesh, new_mesh, grid_points, mod_grid_points, local_grid_points, ll, ul, delta);
        flag = 1;
        iter++;
    ...
    char filename[100];
    sprintf(filename, "red_black_%.3f_%dt.csv", delta, nprocs);
    ofstream MyFile;
