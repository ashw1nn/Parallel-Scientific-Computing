...
char* readFile(const std::string& filename) {
    ...
    if (file.is_open()) {
		...
		{
			text += line + "\n"; // Append the line to msgstr with a newline character
		}
        file.close();
    } else {
...
int main(int argc, char* argv[]) {
	...
	// char message[1024] = "";
	char* message = readFile("plain3.txt");
...
#pragma omp parallel for num_threads(threads) shared(paddedMessageLen, paddedMessage, encryptedMessage, expandedKey) schedule(dynamic)
    ...
    std::cout << "Program execution time: " << duration * 1000000.0 << " microseconds" << std::endl;
	return 0;
}